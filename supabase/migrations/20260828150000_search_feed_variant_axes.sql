-- Inclui variant_axes na busca do feed para expandir anúncios por cor com imagens corretas.

drop function if exists public.search_feed_products(text, uuid[], char(2), jsonb, int, int);
drop function if exists public.search_feed_products(text, uuid[], char(2), int, int);

create function public.search_feed_products(
...
grant execute on function public.search_feed_products(text, uuid[], char(2), jsonb, int, int) to authenticated, service_role;
