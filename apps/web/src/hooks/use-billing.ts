...
import type { CreateCheckoutInput } from '@/services/billing'
...
export function useCreateCheckout() {
  ...
  return useMutation({
    mutationFn: (input: CreateCheckoutInput) => billing.invokeCreateCheckout(input),
    onSuccess: () => {
      if (user?.id) {
        queryClient.invalidateQueries({ queryKey: billingKeys.subscription(user.id) })
      }
    },
  })
}
