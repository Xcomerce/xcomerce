...
const ACTIVE_ROLE_KEY = 'keve.activeRole.buyer'
...
export function AuthProvider({ children }: { children: ReactNode }) {
  ...
  const loadProfile = useCallback(async (userId: string) => {
    ...
    } else if (bundle.roles.includes('buyer')) {
      setActiveRoleState('buyer')
      await writeStoredRole('buyer')
    } else {
  ...
  const signUp = useCallback(async (input: RegisterInput) => {
    await signUpWithEmail(input, 'buyer')
  }, [])
  ...
  const value = useMemo(
    ...
    [session, user, profile, roles, activeRole, isLoading, signIn, signUp, signOut, resetPassword, refreshProfile],
  )
