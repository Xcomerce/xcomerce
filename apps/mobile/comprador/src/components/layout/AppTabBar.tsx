...
import { LayoutGrid, LayoutList, Package, PlusCircle, User } from 'lucide-react-native'

type TabRouteName = 'index' | 'demands' | 'fab-placeholder' | 'orders' | 'profile'

const TAB_ORDER: TabRouteName[] = ['index', 'demands', 'fab-placeholder', 'orders', 'profile']
...
export function AppTabBar({ state, navigation }: BottomTabBarProps) {
  ...
  const navigateTo = (routeName: TabRouteName) => {
    if (routeName === 'fab-placeholder') {
      router.push('/(app)/request-offer')
      return
    }
    if (routeName === 'demands') {
      router.navigate('/(app)/demands')
      return
    }
