...
export function useChatSubscription(demandId: string | undefined, supplierId: string | undefined) {
  const queryClient = useQueryClient()
  ...
  useEffect(() => {
    ...
    const unsubscribe = chat.subscribeMessages(demandId, supplierId, (message) => {
      queryClient.setQueryData<OfferMessage[]>(
        ...
        (prev: OfferMessage[] | undefined) => {
          if (!prev) return [message]
          if (prev.some((m: OfferMessage) => m.id === message.id)) return prev
          return [...prev, message]
        },
      )
    })
