...
import type { Order, OrderRole, OrderStatus, SupplierOrderListItem } from '@/services/orders'
...
export function useOrders(role: OrderRole = 'buyer') {
  ...
  return useQuery({
    ...
    queryFn: () =>
      role === 'supplier'
        ? orders.fetchSupplierOrdersWithDetails(user!.id)
        : orders.fetchOrders(user!.id, role),
    enabled: !!user?.id,
  })
}
  ...
  return useMutation({
    ...
    onSuccess: (data) => {
      ...
      if (user?.id) {
        ...
        queryClient.invalidateQueries({ queryKey: orderKeys.list(user.id, 'supplier') })
      }
    },
  })
}
